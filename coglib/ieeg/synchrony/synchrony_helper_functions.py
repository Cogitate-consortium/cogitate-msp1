...
# -*- coding: utf-8 -*-
"""
Created on Fri Nov 25 23:33:25 2022
...
# methods needed for shuffled connectivity
from mne.epochs import BaseEpochs
from mne.source_estimate import _BaseSourceEstimate
from mne.parallel import parallel_func
from mne_connectivity.base import SpectroTemporalConnectivity, SpectralConnectivity
from mne_connectivity.spectral.epochs import _check_estimators, _get_and_verify_data_sizes, _epoch_spectral_connectivity, _prepare_connectivity, _get_n_epochs, _assemble_spectral_params, _check_option
...
def _epoch_spectral_connectivity_shuffle(data, sig_idx, tmin_idx, tmax_idx, sfreq,
    ...
    for this_data in data:
        if mode in ('multitaper', 'fourier'):
            ...
            # repeat the last sig n_perms

            tmp = np.repeat( np.expand_dims(this_x_t[1, :, :],0), len(idx_map[0])-1, axis=0)
            this_x_t = np.concatenate( (this_x_t, tmp), axis=0)

            
            # shuffle the phases along the time dimension
            random_state = int(np.random.randint(0, 10000, size=1))
            rnd = np.random.RandomState(random_state)
            for sig in range( this_x_t.shape[0] ):
                tr_ = rnd.permutation(this_x_t.shape[-1])
                this_x_t[sig, :, :] = np.abs(this_x_t[sig, :, :])*np.exp( 1j*np.angle(this_x_t[sig, :, :])[:, tr_] )
...
def spectral_connectivity_epochs_shuffle(data, names=None, method='coh', indices=None,
                                 sfreq=None,
                                 ...
                                 cwt_n_cycles=7, block_size=1000, n_jobs=1, n_perms=200,
                                 verbose=None):
    ...
    **Supported Connectivity Measures**
    ...
    if n_jobs != 1:
        parallel, my_epoch_spectral_connectivity, _ = \
            parallel_func(_epoch_spectral_connectivity_shuffle, n_jobs,
                          verbose=verbose)
    ...
    for epoch_block in _get_n_epochs(data, n_jobs):
        if epoch_idx == 0:
            ...
            print('    the following metrics will be computed: %s'
                        % metrics_str)
        ...
        # use precomputed cwt and shuffle
        ...
        if n_jobs == 1:
            ...
            for this_epoch in epoch_block:
                ...
                _epoch_spectral_connectivity_shuffle(data=this_epoch, **call_params)
                epoch_idx += 1
        else:
    ...
    # stack all the permutations
    # conn_list = np.stack( con_perms, axis=3)
