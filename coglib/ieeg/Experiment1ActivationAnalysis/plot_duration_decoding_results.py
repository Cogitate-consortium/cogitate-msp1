"""
This script plots the results of the decoding analysis
...
def plotting_duration_decoding_results(configs, save_folder="super"):
    """
    ...
    for config in configs:
        ...
        for analysis_name, analysis_parameters in param.analysis_parameters.items():
            ...
            for roi in param.rois:
                ...
                results_file = find_files(load_path_results, "*" + roi + "_duration_decoding_accuracy_stats",
                                          extension=".csv")
                assert len(results_file) == 1, "More than one file was found for rsa results!"
                # Load the file:
                duration_decoding_results = pd.read_csv(results_file[0])
                # Load the epochs of each subject:
                sub_epochs = {}
                for subject in duration_decoding_results["subject"].unique():
                ...
                for channel in list(duration_decoding_results["channel"].unique()):
                    # Extracting the results of that channel only:
                    ch_duration_decoding_results = \
                        duration_decoding_results.loc[duration_decoding_results["channel"] == channel]
                    # Generating the path to the save dir of that specific results:
                    if ch_duration_decoding_results["reject"].item():
                        fig_path = Path(save_path_fig, "duration_decoding")
                        if not os.path.isdir(fig_path):
                            os.makedirs(fig_path)
                    else:
                        fig_path = Path(save_path_fig, "no_decoding")
                        if not os.path.isdir(fig_path):
                            os.makedirs(fig_path)
                    ...
                    if ch_duration_decoding_results["reject"].item():
                        plt.suptitle(channel + " " + analysis_parameters["signal"] + " signal" + "\n"
                                     + "duration decoding accuracy: {:.2f}".format(
                            ch_duration_decoding_results["decoding_score"].item()))
                    else:
                        plt.suptitle(channel + " " + analysis_parameters["signal"] + " signal")
                    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    plotting_duration_decoding_results(configs, save_folder="super")
