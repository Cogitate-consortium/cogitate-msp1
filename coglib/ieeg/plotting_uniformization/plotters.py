...
def plot_matrix(data, x0, x_end, y0, y_end, mask=None, cmap=None, ax=None, ylim=None, midpoint=None, transparency=1.0,
                ...
                title=None, square_fig=False, do_cbar=True, interpolation="lanczos", c_contour="k", crop_cbar=False):
    """
    ...
    if ax is None:
        if square_fig:
            ...
            fig, ax = plt.subplots(figsize=[mm2inch(fig_size[0]),
                                            mm2inch(fig_size[1])])
    if ylim is None:
        ylim = [np.nanpercentile(data, 5), np.nanpercentile(data, 95)]
    if midpoint is None:
        midpoint = np.mean([ylim[0], ylim[1]])
    if crop_cbar and ylim[0] != 0:
        try:
            if np.abs(ylim[1]) > np.abs(ylim[0]):
                norm = matplotlib.colors.TwoSlopeNorm(vmin=-ylim[1], vcenter=midpoint, vmax=ylim[1])
            else:
                print("Warning: the upper ylim is lower than the absolute value of the minimum value. "
                      "The color bar will therefore not be cropped!")
    ...
    if mask is not None:
        ...
        if not np.isnan(mask).all():
            ...
            ax.contour(mask > 0, mask > 0, colors=c_contour, origin="lower",
                       extent=[x0, x_end, y0, y_end], linewidths=2)
...
def plot_time_series(data, t0, tend, ax=None, err=None, colors=None, vlines=None, ylim=None,
                     ...
                     patches=None, patch_color="r", patch_transparency=0.2, x_ticks=None):
    """
...
def plot_rasters(data, t0, tend, cmap=None, ax=None, ylim=None, midpoint=None, transparency=1.0,
                 ...
                 title=None, square_fig=False, conditions=None, cond_order=None, do_cbar=True,
                 add_cond_ticks=True, crop_cbar=True):
    """
    This function plots 2D matrices such as temporal generalization decoding or time frequency decompositions with or
    without significance. If a significance mask is passed, the significance pixels will be surrounded with significance
    line. The significance parts will be fully opaque but the non-significant patches transparency can be controlled
    ...
    :param do_cbar: (boolean) whether or not to do the color bar
    :param add_cond_ticks: (boolean) whether or not to add the conditions ticks
    :return:
    """
    if ax is None:
        if square_fig:
    ...
    if midpoint is not None:
        ...
        norm = matplotlib.colors.Normalize(vmin=ylim[0], vmax=ylim[1])
    if cmap is None:
        cmap = def_cmap
    if square_fig:
        aspect = "equal"
...
def plot_brain(subject='fsaverage', subjects_dir=None, surface='inflated', hemi='lh', sulc_map='curv',
               parc='aparc.a2009s', roi_map=None, views=['lateral', 'medial'],
               cmap='Oranges', colorbar=True, colorbar_title='ACC', colorbar_title_position='top', vmin=None, vmax=None,
