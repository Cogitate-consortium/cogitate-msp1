...
def decoding_batch_runner(robustness_checks=False, naming_pattern="*", exclude=None):
    """
    ...
    for config in config_files:
        ...
        if robustness_checks:
            run_command = "sbatch " + "decoding_robustness_job.sh" \
                          + " --config=" \
                          + '"{}"'.format(config)
        else:
            run_command = "sbatch " + "decoding_master_job.sh" \
                          + " --config=" \
                          + '"{}"'.format(config)
...
if __name__ == "__main__":
    ...
    decoding_batch_runner(robustness_checks=False, naming_pattern=args.pattern, exclude=args.exclude)
