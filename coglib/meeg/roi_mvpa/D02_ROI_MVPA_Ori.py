...
def Orientation_WCD(epochs_rs,stcs,conditions_C,select_F,n_trials,roi_name,score_methods,fname_fig):
    ...
    for condi, Cwcd in wcd.items():
        ax.plot(t, gaussian_filter1d(Cwcd,sigma=4), linewidth=3, label=str(condi), path_effects=pe)
    ax.axhline(0.33,color='k',linestyle='--',label='chance')
    ax.axvline(0, color='k')
    ax.legend(loc='upper right')
    ax.set_title(f'WCD_ori_ {roi_name}')
