...
"""
====================
D02. Decoding for MEG on source space of ROI
...
def Orientation_PFC(fpath_fw,rank,common_cov,sub_info,surf_label_list,
    ...
    for n,roi in enumerate(['PFC', 'IIT','IITPFC']):
        clf[roi] = make_pipeline(Vectorizer(),
                                 ...
                                 #(n_components=nPCA),
                                 sklearn.svm.SVC(kernel='linear',class_weight='balanced',probability=True))   #LogisticRegression(),
    ...
    # do the average trial
        ...
        new_x_PFC = np.concatenate((new_x_PFC[0],new_x_PFC[1],new_x_PFC[2]),axis=0)
