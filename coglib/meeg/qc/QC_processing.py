...
def run_qc_processing(subject_id, visit_id, has_eeg):
    ...
    with PdfPages(op.join(qc_output_path, subject_id + '_' + visit_id + '_MEG_QC.pdf')) as pdf:
        #region first page
        ...
        run = 0
        for fname in os.listdir(data_path): #bug - not listed in order #TODO
            if fname.endswith(".fif") and "run" in fname:      
                ...
                if run_part_2:
                    ...
                    if has_eeg:
                        ...
                        ax1 = plt.subplot2grid((3,4), (0,0), colspan=2)
                        plt.axis('off')
                        ax1.text(0, 0.7, 'interpolated: '  + ', '.join(prep.interpolated_channels))
                        ax1.text(0, 0.4, 'remained noisy: '  + ', '.join(prep.still_noisy_channels))
                        pdf.savefig(fig)
                        plt.close()
        ...
        if run_part_3:
            ...
            reject = get_rejection_threshold(epochs, ch_types=['mag', 'grad'], #'eeg'], #TODO: eeg not use for epoch rejection
                                            decim=2)
            ...
            # Get rejection thresholds - EEG only
